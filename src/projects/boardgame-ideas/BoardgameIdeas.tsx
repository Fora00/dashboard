...
import { db, type BoardgameIdea as BoardgameIdeaRow } from '../../lib/db'
import { addBoardgameIdea, deleteBoardgameIdea, sync, updateNotes } from '../../lib/boardgameIdeasSync'
...
export function BoardgameIdeas() {
  ...
  const ideas = useLiveQuery(() => db.boardgameIdeas.orderBy('createdAt').reverse().toArray())
  ...
  async function addIdea(e: FormEvent) {
    ...
    if (!trimmed) return
    await addBoardgameIdea(trimmed)
    setText('')
  }
  ...
  async function saveNotes(idea: BoardgameIdeaRow, notes: string) {
  ...
  async function remove(idea: BoardgameIdeaRow) {
    if (expandedId === idea.id) setExpandedId(null)
    await deleteBoardgameIdea(idea.id)
    trigger(`Deleted "${idea.text}" · Undo`, () => sync.upsert('boardgame_ideas', idea))
  }

  const renderIdea = (idea: BoardgameIdeaRow) => {
    ...
    <div>
      ...
      <form onSubmit={addIdea} className="mb-6 flex gap-2">
        <input
          ...
          placeholder="Add a boardgame idea…"
          autoComplete="off"
          enterKeyHint="done"
